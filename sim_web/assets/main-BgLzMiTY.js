...
const toolbarHintEl = document.getElementById('toolbar-hint');
...
let objectMovedAfterStack = false;
...
function renderCameraViewsToggle() {
  if (cameraFeedGridEl) cameraFeedGridEl.hidden = false;
  overheadCameraFeedEl?.classList.toggle('is-live', true);
  wristCameraFeedEl?.classList.toggle('is-live', true);
}
...
function renderGraspControls() {
  ...
  if (toolbarHintEl) {
    toolbarHintEl.hidden = graspState.running ||
      graspState.stage !== 'complete' ||
      graspState.mode !== 'stack' ||
      objectMovedAfterStack;
  }
...
async function main() {
  ...
  function reportObjectMove(result) {
    if (result.ok) {
      objectMovedAfterStack = true;
      if (toolbarHintEl) toolbarHintEl.hidden = true;
  ...
  window.setTimeout(() => {
    if (startStackEl && !graspState.running) {
      startStackEl.click();
    }
  }, 400);
