...
function geomMaterial(model, index, materialProps, mujoco) {
  ...
  const matName = (matid >= 0 && mujoco)
    ? (namedObject(mujoco, model, mujoco.mjtObj.mjOBJ_MATERIAL?.value, matid, '') || '')
    : (materialProps && materialProps[matid]?.name || '');

  const bodyId = model.geom_bodyid ? model.geom_bodyid[index] : -1;
  const bodyName = (bodyId >= 0 && mujoco)
    ? namedObject(mujoco, model, mujoco.mjtObj.mjOBJ_BODY?.value, bodyId, '')
    : '';
  const geomName = (mujoco)
    ? namedObject(mujoco, model, mujoco.mjtObj.mjOBJ_GEOM?.value, index, '')
    : '';

  const isCnc = matName === 'rs_anodized_silver_mat' || geomName.includes('cnc');
  const isBlack = matName.includes('black') || bodyName.includes('base');
  const isGreenYellow = matName === 'rs_seeed_green_mat' || matName.includes('green') || matName.includes('yellow');
  const isTable = matName === 'rs_table' || bodyName === 'task_table' || geomName.includes('table');
  const isD405 = matName.startsWith('d405_');
  const isD405Lens = matName === 'd405_lens_mat';
  const isMetal = metalness >= 0.45 || isCnc;

  if (isCnc) {
    // Realistic anodized aluminum silver tint matching Picture 2
    color.setHex(0x9aa2ac);
  } else if (isBlack) {
    // Deep charcoal black joints & motor housings
    color.setHex(0x18181c);
  } else if (isGreenYellow) {
    // Vibrant neon safety yellow accents
    color.setHex(0xe2f700);
  } else if (isTable) {
    // Medium slate gray task table (distinct slate gray, NOT white!)
    color.setHex(0x848e98);
  }

  return new THREE.MeshPhysicalMaterial({
    color,
    metalness: isCnc ? 0.85 : isBlack ? 0.25 : isGreenYellow ? 0.08 : isTable ? 0.04 : (Number.isFinite(metalness) ? Math.max(0, Math.min(1, metalness)) : 0.12),
    roughness: isCnc ? 0.24 : isBlack ? 0.42 : isGreenYellow ? 0.32 : isTable ? 0.55 : (Number.isFinite(roughness) ? Math.max(0, Math.min(1, roughness)) : 0.55),
    envMapIntensity: isCnc ? 1.8 : isMetal ? 1.0 : 0.5,
    clearcoat: isCnc ? 0.35 : 0,
    clearcoatRoughness: isCnc ? 0.12 : 0.4,
...
function shouldDrawGeom(model, index, mujoco) {
  ...
  if (mujoco) {
    const name = namedObject(mujoco, model, mujoco.mjtObj.mjOBJ_GEOM?.value, index, '');
    if (name && name.includes('zone_')) return false;
  }
  return true;
}
...
function createGridTexture() {
  ...
  // Transparent background: remove solid white ground block/tile
...
function createBackdropTexture() {
  ...
  sky.addColorStop(0, '#f5f6f8');
  sky.addColorStop(0.5, '#eceef2');
  sky.addColorStop(1, '#e2e5ea');
  ctx.fillStyle = sky;
...
function createEnvironmentTexture() {
  ...
  base.addColorStop(0.2, '#e2e8f0');
  base.addColorStop(0.5, '#94a3b8');
  base.addColorStop(0.8, '#cbd5e1');
  base.addColorStop(1, '#ffffff');
  ctx.fillStyle = base;
  ctx.fillRect(0, 0, width, height);

  // Softbox studio highlights for brushed metallic reflections
  ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
  ctx.fillRect(width * 0.15, height * 0.1, width * 0.3, height * 0.25);
...
export function createSceneView(host, options = {}) {
  ...
  const softbox = new THREE.RectAreaLight(0xffffff, 1.2, 1.6, 1.15);
  softbox.position.set(0.28, -0.58, 1.28);
  softbox.lookAt(0.25, 0, 0.16);
  scene.add(softbox);

  const fillLeft = new THREE.RectAreaLight(0xffffff, 0.8, 0.75, 1.05);
  fillLeft.position.set(-0.58, 0.2, 0.68);
  fillLeft.lookAt(0.23, 0, 0.18);
  scene.add(fillLeft);

  const fillRight = new THREE.RectAreaLight(0xffffff, 0.6, 0.7, 0.9);
  fillRight.position.set(0.86, 0.32, 0.62);
  fillRight.lookAt(0.25, 0, 0.18);
  scene.add(fillRight);
  ...
  const side = new THREE.DirectionalLight(0xffffff, 0.35);
  ...
  function build(mujoco, model, materialProps) {
    ...
    for (let i = 0; i < model.ngeom; i += 1) {
      if (!shouldDrawGeom(model, i, mujoco)) continue;
      ...
      if (type === types.plane) {
        ...
        material = geomMaterial(model, i, materialProps, mujoco);
      }
