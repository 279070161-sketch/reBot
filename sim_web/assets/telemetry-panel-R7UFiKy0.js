...
export function createTelemetryPanel(root) {
  ...
  function update(physicsSample, contactSample) {
    if (!rows.length && barsEl) buildRows(physicsSample.joints);
    if (contactCountEl) contactCountEl.textContent = String(contactSample.activeContacts);
    if (graspForceEl) graspForceEl.textContent = `${contactSample.graspForce.toFixed(1)} N`;
    if (rmsErrorEl) rmsErrorEl.textContent = `${(physicsSample.rmsError * 1000).toFixed(1)} mrad`;
    ...
    if (torqueCanvas) drawSeries(torqueCanvas, history, 'torque', 1);
    if (errorCanvas) drawSeries(errorCanvas, history, 'error', 0.004);
  }
  ...
  function reset() {
    ...
    if (contactCountEl) contactCountEl.textContent = '0';
    if (graspForceEl) graspForceEl.textContent = '0.0 N';
    if (rmsErrorEl) rmsErrorEl.textContent = '0.0 mrad';
    if (torqueCanvas) drawSeries(torqueCanvas, history, 'torque', 1);
    if (errorCanvas) drawSeries(errorCanvas, history, 'error', 0.004);
  }
